...
def _docs_for_action(factory: dict, action: dict, gaps: list[dict]) -> list[dict]:
    """Decide which documents to draft based on the action's target gap."""
    ...
    primary_buyer = buyers[0] if buyers else "Buyer"

    out.append(generate_buyer_email(factory_name, primary_buyer, gap, action.get("title", "")))
