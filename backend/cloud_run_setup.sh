...
# 1. Set project
gcloud config set project exportiq-496416
...
if ! gcloud secrets describe exportiq-sa-key --project=exportiq-496416 >/dev/null 2>&1; then
...
gcloud run deploy exportiq-backend \
  ...
  --set-env-vars GOOGLE_CLOUD_PROJECT=exportiq-496416 \
  --set-env-vars GEMINI_MODEL=gemini-2.5-pro \
  --set-env-vars USE_MOCK_DATA=true \
  --set-env-vars LOG_LEVEL=INFO \
  --set-env-vars FIREBASE_PROJECT_ID=exportiq-496416 \
  --set-env-vars FIREBASE_STORAGE_BUCKET=exportiq-496416.firebasestorage.app \
